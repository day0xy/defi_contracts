...
library UniswapV2Library {
    ...
    function getReserves(
        ...
        (uint reserve0, uint reserve1, ) = IEtherVistaPair(
...
contract EtherVistaRouter is IEtherVistaRouter {
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "EtherVistaRouter: INSUFFICIENT_B_AMOUNT"
                );
                ...
                require(
                    ...
                    "EtherVistaRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        liquidity = IEtherVistaPair(pair).first_mint(
            msg.sender,
            buyLpFee,
            sellLpFee,
            buyProtocolFee,
        ...
        IEtherVistaPair(UniswapV2Library.pairFor(factory, token, WETH))
            .updateProvider(msg.sender);
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IEtherVistaPair pair = IEtherVistaPair(
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            msg.value > usdcToEth(Pair.buyTotalFee()),
            "EtherVistaRouter: INSUFFICIENT_ETH_FEE"
        );
        ...
        (bool sent1, ) = IEtherVistaPair(
            UniswapV2Library.pairFor(factory, path[0], path[1])
        ).payableProtocol().call{value: usdcToEth(Pair.buyProtocolFee())}("");
        (bool sent2, ) = UniswapV2Library
            .pairFor(factory, path[0], path[1])
            .call{value: usdcToEth(Pair.buyLpFee())}("");
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            "EtherVistaRouter: INVALID_PATH"
        );
        require(
            msg.value >= usdcToEth(Pair.sellTotalFee()),
            "EtherVistaRouter: INSUFFICIENT_ETH_FEE"
        );
        ...
        (bool sent1, ) = IEtherVistaPair(
            UniswapV2Library.pairFor(factory, path[0], path[1])
        ).payableProtocol().call{value: usdcToEth(Pair.sellProtocolFee())}("");
        (bool sent2, ) = UniswapV2Library
            .pairFor(factory, path[0], path[1])
            .call{value: usdcToEth(Pair.sellLpFee())}("");
        require(sent1 && sent2, "Failed to send Ether");
