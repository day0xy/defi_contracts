...
contract EtherVistaERC20 is IEtherVistaERC20 {
    ...
    function approve(address spender, uint value) external returns (bool) {
        require(
            spender == IEtherVistaFactory(factory).router(),
            "EtherVistaPair: FORBIDDEN"
        );
...
contract EtherVistaPair is IEtherVistaPair, EtherVistaERC20 {
    ...
    function updateEuler(uint256 liqFee) internal {
        if (euler.length == 0) {
            ...
            euler.push(
                euler[euler.length - 1] + (liqFee * bigNumber) / totalSupply
            );
        }
    }
    ...
    function claimShare() public lock {
        ...
        uint256 share = (Providers[msg.sender].lp *
            (euler[euler.length - 1] - Providers[msg.sender].euler0)) /
            bigNumber;
    ...
    function setFees() external {
        ...
        require(
            msg.sender == creator && block.timestamp - startTime_fees >= 3 days
        );
    ...
    function setProtocol() external {
        ...
        require(
            msg.sender == creator &&
                block.timestamp - startTime_protocol >= 3 days
        );
    ...
    function burn(
        ...
        require(
            block.timestamp - creation_time >= 5 days,
            "EtherVistaPair: FORBIDDEN RUGPULL"
        );
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "EtherVistaPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "EtherVistaPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "EtherVista: INSUFFICIENT_LIQUIDITY"
        );
        ...
        require(
            ...
            "EtherVista: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
